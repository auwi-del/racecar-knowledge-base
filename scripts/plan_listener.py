...
功能：
  ...
  2. 每次启动时自动创建 录制-<MMDD-HHMMSS>/ 目录，本次录制的所有文件存入该目录
  3. 监听 /goal_pose，记录目标点信息
  4. 监听 /initialpose，记录初始位姿设置
  5. 终端实时打印路径分析（点数、长度、弯曲度、曲折比）

保存的文件格式：
  ~/racecar/path_records/录制-<MMDD-HHMMSS>/
  └── plan_<序号>_<时间戳>.csv
...
用法（在开发板上）：
    ...
    可选参数：
      --dir <录制目录名>  自定义本次录制目录名（默认: 录制-MMDD_HHMMSS）
...
输出示例：
    ═══════════════════════════════════════════
    🗺️  /plan 监听器已启动
       💾 本次录制: ~/racecar/path_records/录制-0607_143022/
    ═══════════════════════════════════════════
    ────────────────────────────────────────────
    🎯 收到新航点目标: (3.13, -0.56) 朝向 176.2°
    🗺️  收到新全局路径 | 点数: 287 | 总长: 15.83m
       💾 已保存: 录制-0607_143022/plan_0003_20260607_143022.csv
    ────────────────────────────────────────────
"""
...
def format_timestamp_for_filename():
    """生成文件名用的完整时间戳: YYYYMMDD_HHMMSS"""
    return datetime.now().strftime("%Y%m%d_%H%M%S")


def format_session_dirname():
    """生成会话目录名: 录制-MMDD_HHMMSS"""
...
class PlanListener(Node):
    ...
    def __init__(self, save_dir=None, enable_save=True, session_name=None):
        ...
        if self.enable_save and save_dir:
            # 创建本次录制的会话目录
            final_session_name = session_name
            if final_session_name is None:
                final_session_name = format_session_dirname()

            self.save_dir = make_session_dir(save_dir, final_session_name)
            self.session_name = final_session_name

            self.get_logger().info(f"   💾 本次录制: {self.save_dir}/")
    ...
    def plan_callback(self, msg: Path):
        ...
        if self.enable_save and self.save_dir:
            ...
            # 显示相对路径（相对于 save_dir 的父目录）
            rel_path = os.path.relpath(filepath, os.path.dirname(self.save_dir))
            self.get_logger().info(f"    💾 已保存: {rel_path}")
    ...
    def _save_plan_to_csv(self, poses, n_points, total_len, detour_ratio,
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            if self.session_name:
                f.write(f"# 录制: {self.session_name}\n")
...
def main():
    ...
    if args.no_save:
        ...
        enable_save = True
        session_name = args.dir  # None 时由 PlanListener 自动生成
    ...
    try:
        ...
        if enable_save and node.save_dir:
            node.get_logger().info(f"   路径文件保存在: {node.save_dir}/")
    finally:
        node.destroy_node()
        rclpy.shutdown()
